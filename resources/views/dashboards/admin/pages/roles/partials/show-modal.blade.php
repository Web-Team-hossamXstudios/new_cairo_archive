        ...
        <div class="modal-content border-0 shadow" style="border-radius: var(--ins-border-radius);">
            <div class="modal-header border-0 pb-0">
                <div>
                    <h4 class="modal-title fw-bold" id="showModalLabel">
                        <i class="ti ti-shield-check me-2 text-primary"></i>View Role
                    </h4>
                    <p class="text-muted mb-0">View role details and assigned permissions</p>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ x_('Close', 'dashboards.admin.pages.user.roles.partials.show-modal') }}"></button>
            </div>
            <div class="modal-body pt-3" id="showContent" style="max-height: 70vh; overflow-y: auto;">
                <!-- Loading State -->
                <div class="text-center py-5" id="showLoading">
                    <div class="spinner-border text-primary" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>
                    <p class="text-muted mt-2 mb-0">Loading role details...</p>
                </div>
                ...
                <div id="showData" class="d-none">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-md bg-primary-subtle text-primary rounded me-3 d-flex align-items-center justify-content-center">
                                            <i class="ti ti-shield fs-4"></i>
                                        </div>
                                        <div>
                                            <small class="text-muted d-block">{{ x_('Role Name', 'dashboards.admin.pages.user.roles.partials.show-modal') }}</small>
                                            <h5 class="mb-0 fw-semibold" id="showRoleName">-</h5>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-md bg-success-subtle text-success rounded me-3 d-flex align-items-center justify-content-center">
                                            <i class="ti ti-key fs-4"></i>
                                        </div>
                                        <div>
                                            <small class="text-muted d-block">{{ x_('Total Permissions', 'dashboards.admin.pages.user.roles.partials.show-modal') }}</small>
                                            <h5 class="mb-0 fw-semibold" id="showPermissionCount">0</h5>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-md bg-info-subtle text-info rounded me-3 d-flex align-items-center justify-content-center">
                                            <i class="ti ti-users fs-4"></i>
                                        </div>
                                        <div>
                                            <small class="text-muted d-block">{{ x_('Users with Role', 'dashboards.admin.pages.user.roles.partials.show-modal') }}</small>
                                            <h5 class="mb-0 fw-semibold" id="showUserCount">0</h5>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card border border-secondary border-opacity-10 shadow-sm" style="border-radius: var(--ins-border-radius);">
                        <div class="card-header bg-light-subtle py-2 d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-semibold"><i class="ti ti-lock me-2 text-primary"></i>Assigned Permissions</h6>
                            <span class="badge bg-primary shadow-sm" id="showTotalPermissionsCount">0 permissions</span>
                        </div>
                        <div class="card-body p-2">
                            <!-- Permissions Accordion -->
                            <div class="accordion accordion-flush" id="showPermissionsAccordion">
                                    ...
                                    <div class="accordion-item border-0 mb-2">
                                        ...
                                        <h2 class="accordion-header">
                                            <button class="accordion-button collapsed py-2 px-3 shadow-sm" type="button"
                                                data-bs-toggle="collapse" data-bs-target="#show_{{ $mainModuleId }}"
                                                aria-expanded="false" aria-controls="show_{{ $mainModuleId }}"
                                                ...
                                                <div class="d-flex align-items-center justify-content-between w-100 me-3">
                                                    ...
                                                    <span class="badge bg-secondary-subtle text-secondary show-module-count" data-main-module="{{ $mainModuleId }}">
                                                        0 / {{ $moduleData['total_count'] }}
                                                    </span>
                                                </div>
                                            </button>
                                        </h2>
                                        <div id="show_{{ $mainModuleId }}" class="accordion-collapse collapse" data-bs-parent="#showPermissionsAccordion">
                                                            ...
                                                            <div class="card-header py-2 px-3 bg-white d-flex justify-content-between align-items-center" style="border-radius: var(--ins-border-radius) var(--ins-border-radius) 0 0;">
                                                                <div class="d-flex align-items-center">
                                                                    <a class="text-decoration-none text-dark fw-medium d-flex align-items-center"
                                                                       data-bs-toggle="collapse" href="#show_sub_content_{{ $subCategoryId }}" role="button">
                                                                        <i class="ti ti-chevron-right me-1 transition-transform collapse-chevron"></i>
                                                                        <span class="text-primary">{{ $subCategory }}</span>
                                                                    </a>
                                                                </div>
                                                                <span class="badge bg-light text-muted border show-sub-count" data-sub-module="{{ $subCategoryId }}">
                                                                    0 / {{ $subData['total_count'] }}
                                                                </span>
                                                            </div>
                                                            <div class="collapse" id="show_sub_content_{{ $subCategoryId }}">
                                                                    ...
                                                                    @foreach($subData['modules'] as $moduleName => $modulePermissions)
                                                                        ...
                                                                        <div class="mb-3 pb-2 border-bottom border-secondary border-opacity-10">
                                                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                                                ...
                                                                                <span class="badge bg-white text-muted border small show-group-count" data-group="{{ $moduleId }}">
                                                                                ...
                                                                                @foreach($modulePermissions as $permission)
                                                                                    <div class="col-6 col-md-4 col-lg-3">
                                                                                        <div class="form-check mb-1">
                                                                                            <input type="checkbox"
                                                                                                class="form-check-input show-permission-item"
                                                                                                data-main-module="{{ $mainModuleId }}"
                                                                                                data-sub-module="{{ $subCategoryId }}"
                                                                                                data-group="{{ $moduleId }}"
                                                                                                data-permission="{{ $permission->name }}"
                                                                                                id="show_perm_{{ $permission->id }}"
                                                                                                style="width: 14px; height: 14px;"
                                                                                                disabled>
                                                                                            <label class="form-check-label small" for="show_perm_{{ $permission->id }}">
                                                                                                @php
                                                        ...
                                                        <div class="card border border-secondary border-opacity-10 mb-2" style="border-radius: var(--ins-border-radius);">
                                                            <div class="card-body py-2 px-3">
                                                                <div class="d-flex align-items-center justify-content-between mb-2">
                                                                    ...
                                                                    <span class="badge bg-light text-muted border show-group-count" data-group="{{ $moduleId }}">
                                                                        0 / {{ $modulePermissions->count() }}
                                                                    </span>
                                                                </div>
                                                                <div class="row g-1 ms-2">
            ...
            <div class="modal-footer border-0 bg-light-subtle" style="border-radius: 0 0 var(--ins-border-radius) var(--ins-border-radius);">
                <button type="button" class="btn btn-light shadow-sm px-4" style="border-radius: var(--ins-border-radius);" data-bs-dismiss="modal">
                    <i class="ti ti-x me-1"></i>{{ x_('Close', 'dashboards.admin.pages.user.roles.partials.show-modal') }}</button>
            </div>
        </div>
    </div>
</div>
...
<style>
#showModal .accordion-button:not(.collapsed) {
    background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%) !important;
    color: #1976d2;
}
#showModal .accordion-button:focus {
    box-shadow: none;
}
#showModal .form-check-input:checked {
    background-color: black;
    border-color: #212529 !important;
}
#showModal .form-check-input {
    border: 2px solid #6c757d;
}
#showModal .form-check-input:disabled {
    opacity: 0.8;
}
#showModal .show-permission-item:checked + label {
    color: var(--bs-primary);
    font-weight: 500;
}
#showModal .collapse-chevron {
    transition: transform 0.2s ease;
}
#showModal [aria-expanded="true"] .collapse-chevron {
