                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ms-3">
                                        <h4 class="mb-0">{{ \App\Models\File::mainFiles()->processing()->count() }}</h4>
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ms-3">
                                        <h4 class="mb-0">{{ \App\Models\File::mainFiles()->completed()->count() }}</h4>
                            ...
                            <div class="row d-flex align-items-end justify-content-start">
                                ...
                                <div class="col-md-3 mb-2 d-flex align-items-center gap-2">
                                    <div class="d-flex flex-wrap gap-1">
                                        ...
                                        <a href="{{ route('admin.files.index') }}" style="border-radius: var(--ins-border-radius);"
                        ...
                        <div class="card" style="border-radius: var(--ins-border-radius);">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <span class="badge bg-primary-subtle text-primary">{{ $files->total() ?? 0 }} ملف</span>
                                </div>
                                        ...
                                        <tbody>
                                            @forelse($files ?? [] as $file)
                                                <tr id="file-row-{{ $file->id }}">
                                                    <td><input type="checkbox" class="form-check-input row-checkbox" value="{{ $file->id }}"></td>
                                                    <td>
                                                        <span class="badge bg-dark font-monospace">{{ $file->barcode ?? '-' }}</span>
                                                    </td>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <div class="avatar avatar-sm bg-danger-subtle text-danger rounded me-2">
                                                    ...
                                                    <td><span class="badge bg-secondary">{{ $file->pages_count ?? 0 }}</span></td>
                                                    <td>
                                                        @switch($file->status)
                                                            @case('pending')
                                                    ...
                                                    <td>
                                                        <div class="d-flex justify-content-center gap-1">
                                                            ...
                                                            @if($hasDocument)
                                                                <button class="btn btn-soft-info btn-sm" onclick="showFile({{ $file->id }})" title="عرض">
                                                                    <i class="ti ti-eye"></i>
                                                                </button>
                                                            @else
                                                                @can('files.upload')
                                                                <button class="btn btn-soft-primary btn-sm" onclick="openUploadForFile({{ $file->id }}, '{{ $file->file_name }}')" title="رفع ملف">
                                                            ...
                                                            @if($file->barcode)
                                                                <button class="btn btn-soft-dark btn-sm" onclick="printBarcode('{{ $file->barcode }}', '{{ $file->file_name }}')" title="طباعة الباركود">
                                                                    <i class="ti ti-barcode"></i>
                                                                </button>
                                                            @endif

                                                            @can('files.upload')
                                                                <button class="btn btn-soft-warning btn-sm" onclick="editFile({{ $file->id }})" title="تعديل">
                                                                    <i class="ti ti-edit"></i>
                                                                </button>
                                                            @endcan
                                                            @can('files.delete')
                                                                <button class="btn btn-soft-danger btn-sm" onclick="deleteFile({{ $file->id }}, '{{ $file->file_name }}')" title="حذف">
                                                ...
                                                <div class="card border shadow-sm h-100">
                                                    <div class="card-body">
                                                        <div class="d-flex align-items-center mb-3">
                                                            <div class="avatar avatar-md bg-danger-subtle text-danger rounded-circle me-3 d-flex align-items-center justify-content-center">
                                                                <i class="ti ti-file-type-pdf"></i>
                                                            </div>
                                                            <div class="flex-grow-1">
                                                                <h6 class="mb-0">{{ Str::limit($file->original_name ?? $file->file_name, 18) }}</h6>
                                                                <small class="text-muted">{{ $file->barcode ?? '-' }}</small>
                                                            </div>
                                                            <input type="checkbox" class="form-check-input row-checkbox" value="{{ $file->id }}">
                                                        </div>
                                                        <div class="mb-2">
                                                            <small class="text-muted d-block"><i class="ti ti-user me-1"></i> {{ $file->client->name ?? '-' }}</small>
                                                            <small class="text-muted d-block"><i class="ti ti-map-2 me-1"></i> {{ $file->land->land_no ?? '-' }}</small>
                                                        </div>
                                                        <div class="d-flex gap-2 mb-3">
                                                            <span class="badge bg-secondary"><i class="ti ti-file me-1"></i>{{ $file->pages_count ?? 0 }} صفحة</span>
                                                            @switch($file->status)
                                                    ...
                                                    <div class="card-footer bg-transparent border-top-0 pt-0">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            @if($hasDocument)
                                                                <button class="btn btn-soft-info btn-sm" onclick="showFile({{ $file->id }})"><i class="ti ti-eye"></i></button>
                                                            @else
                                                                <button class="btn btn-soft-primary btn-sm" onclick="openUploadForFile({{ $file->id }}, '{{ $file->file_name }}')"><i class="ti ti-upload"></i></button>
                                                            @endif
                                                            <button class="btn btn-soft-warning btn-sm" onclick="editFile({{ $file->id }})"><i class="ti ti-edit"></i></button>
                                                            <button class="btn btn-soft-danger btn-sm" onclick="deleteFile({{ $file->id }}, '{{ $file->file_name }}')"><i class="ti ti-trash"></i></button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
